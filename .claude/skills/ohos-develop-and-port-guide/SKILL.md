...
**Critical: `ASSETS.get()` hang instead of error return.** `rust-embed` in debug mode calls `std::fs::read()` on the filesystem path. Since the asset files DO NOT exist on the device (only on the build machine), `std::fs::read()` blocks indefinitely on a nonexistent file rather than returning a quick error. This manifests as an **app hang** during any operation that loads assets (e.g., font loading in `build_appearance()` → `Cache::handle(ctx).update()` → `load_family_from_bytes()` blocks forever). The app doesn't crash—it deadlocks.
...
**Verify:** After enabling `debug-embed`, check that `libwarp.so` size increases significantly (e.g., from ~790MB to ~916MB for debug build with Hack/Roboto/password fonts embedded). The HAP size also increases correspondingly.

Release builds always embed data, so `--release` also works around this issue (but with stripped symbols).
...
**经验教训（2026-07-20）：** OHOS 上物理鼠标/触控板的同一输入会产生 **三条并行事件流**：

```
物理鼠标/触控板输入
  ├─ onMouse(Press/Move/Release) → OhosEvent::MouseEvent   ✅ 选词
  ├─ onTouch(Down/Move/Up)       → OhosEvent::TouchEvent   ❌ 引起滚动
  └─ NDK dispatch_touch_event    → OhosEvent::TouchEvent   ❌ 误触右键菜单
```

**导致的问题：** 鼠标选词时滚动、滚动条拖拽释放弹右键菜单、触摸滑动后误触发点击。

**解决方案：**
1. 禁用 NDK dispatch_touch_event：`InputEvent::TouchEvent => OhosEvent::UserEvent`
2. ArkTS 端 mousePressed/mouseWasActive 守卫（onMouse 设标记，onTouch Move/Up 被守卫跳过）
3. 移除 winit 后端冗余的 MOUSE_PRESSED 过滤

**关键原则：** 消除重复事件源（禁用 NDK TouchEvent），而非在接收端过滤。

**涉及文件：** Index.ets、winit/src/platform_impl/ohos/mod.rs


### 触摸事件来源识别（Mouse vs Touchscreen）

**NDK 提供了两组 API 可用于区分触摸事件的来源：**

| API | 粒度 | 返回值 | 当前使用情况 |
|-----|------|--------|------------|
| `OH_NativeXComponent_GetTouchPointToolType(xc, pointId, &tool)` | 每个触摸点 | Finger / Mouse / Pen / ... | ✅ 已在 `dispatch_touch_event` 中调用，填充 `TouchPointData.event_tool_type` |
| `OH_NativeXComponent_GetTouchEventSourceType(xc, pointId, &source)` | 每个触摸点 | Unknown / Mouse / Touchscreen / Touchpad / ... | ❌ 未使用 |

**关键路径：** `ohos-xcomponent-binding` crate 的 `native_callbacks.rs:dispatch_touch_event()` 中已查询 `GetTouchPointToolType`：
...
因此 `TouchEventData` 到达上层回调时，`data.touch_points[0].event_tool_type` 已含有正确值。

**实践发现（2026-07-20）：** 在测试设备上，物理鼠标产生的合成 TouchEvent 的 `event_tool_type` 返回的是 `Finger` 而非 `Mouse`，无法区分真实触摸与鼠标合成事件。这可能因 OHOS 版本或设备驱动而异。

**推荐策略：** 不要在 NDK TouchEvent 路径上依赖 tool type 或 source type 做过滤。改用 ArkTS 层的 `onMouse`/`onTouch` 事件顺序和状态标记来判断：

```
mouseWasActive 标记法:
  1. onMouse(Press) 时设置 mouseWasActive = true
  2. onTouch(Down) 时若 !mousePressed 则重置 mouseWasActive = false
  3. onTouch(Up) 时判断 mouseWasActive——false 才发给 NDK
```

**如果需要使用 `GetTouchEventSourceType`：**
1. 在 `ohos-xcomponent-sys` 中声明 FFI 函数（已存在）
2. 在 `dispatch_touch_event` 或 `ability_event_to_ohos_event` 中调用并判断
3. `source == 1 (Mouse)` 或 `source == 3 (Touchpad)` 时跳过事件
...
## ArkTS 模块集成（@ohos-rs/ability）

### 问题

HAP 项目的 ArkTS 代码中 `import { DefaultXComponent } from '@ohos-rs/ability'` 需要编译器能找到 `@ohos-rs/ability` 模块的定义。该模块源码在 `depends/openharmony-ability/native_ability/` 下。

有三种集成方式：

### 方式一：模块注册（推荐，无代码复制）

在 HAP 的 `build-profile.json5` 中将 `native_ability` 注册为项目模块：
...
"modules": [
  ...
  {
    "name": "native_ability",
    "srcPath": "../depends/openharmony-ability/native_ability"
  }
]
```

同时在 `entry/oh-package.json5` 中声明依赖：

```json
{
  "dependencies": {
    "@ohos-rs/ability": "file:../../depends/openharmony-ability/native_ability"
  }
}
```

**原理**：`build-profile.json5` 的模块注册让 hvigor 把 `native_ability` 纳入编译体系，`oh-package.json5` 的 `file:` 依赖让 ArkTS 编译器知道 `@ohos-rs/ability` 解析到哪个模块。源码只存在于 `depends/` 一处，**没有复制、没有软链接**。

### 方式二：复制到 oh_modules

在没有 `ohpm` 的环境下，可将 `native_ability` 复制到 `oh_modules/@ohos-rs/ability/`。注意删除 `build-profile.json5`、`hvigorfile.ts` 等构建配置文件避免冲突。

### 方式三：使用 package/ 目录

`openharmony-ability/package/` 是纯 ArkTS 源码包（`index.ets` + `src/main/ets/`），不包含构建配置。但该目录**缺少实际的源码文件**（`index.ets` 引用的子模块在 `native_ability/` 中），不能单独使用。

### 注意事项

- 共享文件系统不支持软链接，`deps/` 下的软链接方案不可用
- `file:` 依赖需先运行 `ohpm install` 才能安装到 `oh_modules/`，没有 ohpm 时模块注册方式是唯一零复制的方案
- `native_ability/` 目录包含 C++ 源码（`src/main/cpp/`），在 HAP 构建时由 CMake 编译为 `libnative_ability.so`

## Debugging

hilog 必须按 `com.warp_ohos.terminal` 过滤：
```bash
timeout 5 hdc hilog 2>&1 | grep "com.warp_ohos.terminal" | grep "MUSL-LDSO\|MMG.*warp"
timeout 5 hdc hilog -l I 2>&1 | grep "com.warp_ohos.terminal/Warp\|com.warp_ohos.terminal/napi_init"
```

清空缓存后启动：
```bash
hdc shell "hilog -r" && sleep 1 && hdc shell "aa start -a EntryAbility -b com.warp_ohos.terminal"
```

查 app PID：
```bash
hdc shell ps -ef | grep com.warp_ohos.terminal | grep -v grep
```
...
**涉及修改的文件**：
- `depends/openharmony-ability/crates/ability/src/app.rs` — 新增 `OpenHarmonyApp::surface_size()` 方法
