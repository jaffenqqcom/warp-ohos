...
do_git_repo_tag() {
    ...
    for patch in "$@"; do
        ...
        echo "  → 应用 patch: $patch"
        if ! git apply --3way "$PATCH_DIR/$patch"; then
...
do_crate() {
    ...
    if [ -f "$patch_file" ] && [ -s "$patch_file" ]; then
        ...
        if ! git apply -p"$p_strip" "$patch_file"; then
...
run "winit"               do_git_repo "$BASE_DIR/winit"    "https://github.com/warpdotdev/winit.git"               "a4e0ecb5f9626ccac9445a73dc28354b52423abc" "winit"               "winit/00-tracked.patch" "winit/01-new-files.patch"
run "openharmony-ability" do_git_repo "$BASE_DIR/openharmony-ability" "https://github.com/harmony-contrib/openharmony-ability.git" "6c52bb44164ea2d6d7f573c090a75142f0dbd2ef" "openharmony-ability" "openharmony-ability/00-tracked.patch" "openharmony-ability/01-new-files.patch"
run "nix"                 do_crate "nix" "0.26.4"
run "interprocess"        do_crate "interprocess" "1.2.1"
run "gettext-sys"         do_crate "gettext-sys" "0.21.3"
