// §2.2.6 — 显示信息服务
//
// NAPI 方法 getDisplayInfo()，
// 通过 display.getDefaultDisplaySync() 获取屏幕尺寸和密度。
//
// C FFI 桥（ohos_get_display_info）由 Rust 侧通过 extern "C" 调用。
// 使用 TSFN 桥接在 UI 线程安全调用 NAPI，避免跨线程 abort。
...
static napi_value GetDisplayInfo(napi_env env, napi_callback_info info) {
    napi_value display;
    napi_value defaultDisplay;
    napi_value width;
    napi_value height;
    napi_value density;
    napi_value orientation;

    // OHOS: display.getDefaultDisplaySync() 返回 Display 对象
    // 使用 requireNapi 导入模块
    napi_value displayModule;
    ...
    napi_set_named_property(env, obj, "density", density);
    napi_set_named_property(env, obj, "orientation", orientation);
    return obj;
}

void RegisterDisplay(napi_env env, napi_value exports) {
    OH_LOG_INFO(LOG_APP, "Display::Register called");
    napi_property_descriptor desc[] = {
        {"getDisplayInfo", nullptr, GetDisplayInfo, nullptr, nullptr, nullptr, napi_default, nullptr},
    };
...
const char* ohos_get_display_info() {
    static thread_local std::string buf;
    char result[4096] = {0};
    ohos_call_napi("getDisplayInfo", result, sizeof(result));
    buf = result;
    return buf.c_str();
}
